...
<li class="breadcrumb-item">
  <a href="{{ route('employee.notifications.index') }}">{{ __('Notifications') }}</a>
</li>
...
<div class="card">
  <div class="card-header header-elements d-flex justify-content-between align-items-center">
    ...
    {{-- <form class="form d-flex align-items-center" method="GET" action="#">
      <a href="{{ route('employee.notifications.create') }}" class="btn btn-primary" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="tooltip-primary" title="{{ __('Add New') }}">
    ...
    <table class="table table-hover data-table">
      ...
      <tbody class="table-border-bottom-0">
        @php
            ...
            $data = auth('employee')->user()->notifications()
                ->latest()
                ->paginate($per_page);

            // Update last read timestamp
            $employee->notifications()->update(['status' => 1]);
        @endphp
