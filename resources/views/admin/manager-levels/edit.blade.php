...
@section('title', 'Edit Manager Level')

@section('content-header', __('Edit Manager Level'))
  ...
  <div class="col-md-8 offset-md-2">
    <div class="card">
      <div class="card-header">
        <h5 class="mb-0">{{ __('Edit Manager Level') }}: {{ $managerLevel->name }}</h5>
        @if($managerLevel->is_predefined)
          <span class="badge bg-warning mt-2">{{ __('Predefined Level') }}</span>
        @endif
      </div>
      <div class="card-body">
        <form action="{{ route('admin.manager-levels.update', $managerLevel) }}" method="POST">
          @csrf
          @method('PUT')
          ...
          <div class="row">
            <div class="col-md-6 mb-3">
              ...
              <input type="text" class="form-control @error('name') is-invalid @enderror" 
                     id="name" name="name" value="{{ old('name', $managerLevel->name) }}" 
            ...
            <div class="col-md-6 mb-3">
              ...
              <input type="text" class="form-control @error('code') is-invalid @enderror" 
                     id="code" name="code" value="{{ old('code', $managerLevel->code) }}" 
          ...
          <div class="row">
            <div class="col-md-6 mb-3">
              ...
              <input type="number" class="form-control @error('hierarchy_level') is-invalid @enderror" 
                     id="hierarchy_level" name="hierarchy_level" value="{{ old('hierarchy_level', $managerLevel->hierarchy_level) }}" 
            ...
            <div class="col-md-6 mb-3">
              <div class="form-check mt-4">
                <input class="form-check-input" type="checkbox" id="status" name="status" 
                       {{ old('status', $managerLevel->status) ? 'checked' : '' }}>
          ...
          <div class="mb-3">
            ...
            <textarea class="form-control @error('description') is-invalid @enderror" 
                      ...
                      placeholder="Describe the responsibilities and scope of this level">{{ old('description', $managerLevel->description) }}</textarea>
            @error('description')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          @if($managerLevel->managers()->count() > 0)
            <div class="alert alert-info">
              ...
              {{ __('This level is currently used by') }} <strong>{{ $managerLevel->managers()->count() }}</strong> {{ __('manager(s)') }}.
              {{ __('Changes will affect all managers using this level.') }}
            </div>
          @endif
          ...
          <div class="card mt-4">
            <div class="card-body bg-light">
              ...
              <div class="row">
                @php
                  $existingLevels = \App\Models\ManagerLevel::where('id', '!=', $managerLevel->id)->orderBy('hierarchy_level')->get();
                @endphp
          ...
          <div class="d-flex justify-content-between mt-4">
            ...
            <button type="submit" class="btn btn-primary">
              <i class="bx bx-save me-1"></i> {{ __('Update Level') }}
            </button>
          </div>
        </form>
      </div>
