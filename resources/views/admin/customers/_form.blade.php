<form id="customerForm" class="card-body" action="{{ $_route }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3 col-6">
            ...
            <select class="form-select" id="agency_id" name="agency_id" required>
                ...
                @foreach($agencies as $agency)
                    <option value="{{ $agency->id }}" @selected($agency->id == $customer->agency_id)>{{ $agency->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-3 col-6">
    ...
    <div class="pt-4">
        ...
        <a href="{{route('admin.customers.index')}}" class="btn btn-label-secondary">{{ __('Cancel') }}</a>
    </div>
</form>
